app.factory('factoryMarker',function(factoryGetDevices, map, $compile){
    ...
    function divIcon(name, i){
        var template = '<marker name="'+name+'" device="factoryGetDevices['+i+']"></marker>';
    ...
    function marker(i){
        ...
        devices[i]._marker = L.marker([f(devices[i].lat),f(devices[i].lng)],{icon: divIcon(devices[i].text,i)}).addTo(map.map);
